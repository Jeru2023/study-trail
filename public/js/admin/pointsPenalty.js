...
const PAGE_DIRECTION = 'penalty';

const TEXT = {
  loadingStudents: '正在加载积分数据...',
  refreshSuccess: '积分信息已刷新',
  historyRefreshSuccess: '积分记录已更新',
  adjustSuccess: '扣分成功，积分已更新',
  adjustInProgress: '正在提交扣分...',
  adjustInvalid: '请输入大于 0 的整数积分',
  selectStudent: '请先选择学生账号',
  presetsLoading: '正在加载减分模版...',
  presetHint: '套用常用扣分模版可快速完成操作。',
  presetsEmpty: '还没有减分模版，请先前往配置中心添加。',
  presetApplyInProgress: (name) => `正在应用模版「${name}」...`,
  presetApplySuccess: (name, points) => `已扣除 ${points} 分（模版：${name}）。`
};

const sidebarRoot = qs('[data-component="parent-sidebar"]');
renderParentSidebar(sidebarRoot, { activeKey: 'points:penalty' });
...
async function refreshUnreadNotifications() {
  try {
    ...
    console.error('[points-penalty] failed to refresh unread notifications', error);
  }
}
...
function renderPresetButtons(presets) {
  ...
  if (!presets.length) {
    elements.presetList.innerHTML = '<span class="points-preset__empty">暂无减分模版</span>';
    return;
  }
...
async function loadPointPresets({ silent = false } = {}) {
  ...
  try {
    ...
    const penaltyPresets = normalized.filter((preset) => preset.direction === PAGE_DIRECTION);
    renderPresetButtons(penaltyPresets);
    if (elements.presetHint) {
      elements.presetHint.textContent = penaltyPresets.length
        ? TEXT.presetHint
        : TEXT.presetsEmpty;
    }
    if (!silent && elements.message) {
...
async function handlePresetApply(presetId) {
  ...
  try {
    ...
    setMessage(elements.message, TEXT.presetApplySuccess(note || '该模版', preset.points), 'success');
